...
namespace BrowserChooser.Forms.Views
{
    public partial class BrowserButtonUserControl : UserControl
    {
        ...
        {
            browserIconPictureBox.Image = Browser.Icon?.ToBitmap();
            browserTitleLabel.Text = Browser.Name;

            var browserToolTipString = $"Open {Browser.Name}.\r\nHotkeys: ({Browser.BrowserNumber}) or ({Browser.Name.Substring(0, 1)}).";

            var browserToolTip = new ToolTip();
            browserToolTip.SetToolTip(browserIconPictureBox, browserToolTipString);
            browserToolTip.SetToolTip(browserTitleLabel, browserToolTipString);
        }
